...
RAF 루프는 파티클이 있을 때만 실행된다(`isRunningRef`로 상태 추적). `emit`·`burst` 호출 시 루프를 시작하고, 모든 파티클의 life가 0이 되면 루프를 멈춘다. 마운트 시점에 RAF를 상시 실행하던 기존 방식 대비 긁기 전후 불필요한 CPU 사용을 제거했다.

클립보드 복사 버튼의 `setTimeout` ID를 `useRef`로 보관해 모달 unmount 시 `clearTimeout`으로 정리한다. 연속 복사 시에도 이전 타이머를 먼저 취소해 상태 충돌을 방지한다.
